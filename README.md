Hi, I'm Jean-Louis 👋

I am an aspiring Cybersecurity and IT professional based in Melbourne, Australia.

After nearly 20 years in hospitality management, I made the decision to transition into a long-term career in Information Technology. I recently completed a Diploma of Cybersecurity and Advanced Networking and the Google IT Support Professional Certificate.

I am continuing to build my technical knowledge through hands-on labs, industry certifications, and further study in cybersecurity.

🎯 Career Focus

Cybersecurity

IT Support and Service Desk

Systems Administration

Security Operations

🛠️ Technical Skills

Systems and Administration

Windows 10 and Windows 11

Windows Server 2022

Active Directory Domain Services

Group Policy

Microsoft 365

User and permission management

Windows deployment and troubleshooting

Networking

DNS

DHCP

TCP/IP

VPN and remote access

Network troubleshooting

Routing and firewall rules

pfSense

Virtualisation and Security

VMware

Virtual machines and lab environments

Basic SIEM and security monitoring

Firewall configuration

Access control

Security policies

Incident investigation fundamentals

🎓 Education and Certifications

Diploma of Cybersecurity and Advanced Networking Monarch Institute — Completed July 2026

Google IT Support Professional Certificate

Bachelor of Cyber Security Commencing soon

Currently preparing for additional industry certifications

💻 Hands-On Experience

Through practical labs and assessment projects, I have configured and troubleshot:

Active Directory users, groups, organisational units, and permissions
Windows Server roles and services
DNS and DHCP environments
Group Policy Objects
Remote-access VPNs
pfSense firewall and network security rules
VMware virtual lab environments
Microsoft 365 administration
Windows updates, patching, and support tasks
Network connectivity and access issues
📂 Featured Project
Custom PC Build

A documented custom computer build covering:

Hardware component selection
PC assembly
Windows 11 installation
Driver installation
BIOS configuration
Hardware troubleshooting
System setup and testing
🚀 Projects in Development

I am currently building a practical GitHub portfolio that will include:

Windows Server and Active Directory lab
DNS, DHCP, and Group Policy lab
pfSense firewall configuration lab
VPN and remote-access lab
Cybersecurity monitoring exercises
IT support troubleshooting guides
Network diagrams and documentation

🤝 Transferable Skills

My background in hospitality management helped me develop strong professional skills, including:

Customer service
Leadership
Team management
Problem-solving
Communication
Conflict resolution
Prioritization
Working under pressure
Training and supporting team members

I am now combining these skills with my technical knowledge to build a long-term career in cybersecurity and information technology.

🌱 Currently Learning
Cybersecurity fundamentals
Microsoft Azure
Microsoft 365 administration
Security monitoring
Networking and system administration
PowerShell
Linux fundamentals

📫 Connect With Me

Location: Melbourne, Australia
GitHub: Jean-Louis05
LinkedIn: www.linkedin.com/in/jean-louis-nseir-694a4619b


- ⚡ Fun fact: ...
-->
